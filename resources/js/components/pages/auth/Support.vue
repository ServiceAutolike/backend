                        ...
                        <div class="d-flex flex-wrap flex-stack">
                            ...
                            <div class="d-flex my-2">
                                ...
                                <div class="d-flex align-items-center position-relative me-4">
                                    ...
                                    <!--end::Svg Icon-->
                                    <input type="text" @keyup="searchName($event)" v-model="textSearch" class="form-control form-control-sm w-150px ps-9 py-4" placeholder="Nhập mã hỗ trợ" />
                                </div>
                                <div class="d-flex align-items-center position-relative me-4">
                                    <div class="form-floating">
                                        ...
                                        <label for="floatingSelect">Lọc theo loại hỗ trợ</label>
                                    </div>
                                </div>
                                <!--end::Search-->
                            </div>
                    ...
                    <div v-else class="table-responsive">
                        ...
                        <table class="table table-flush align-middle table-row-bordered table-row-solid gy-4 gs-9">
                            ...
                            <tr v-for="data in dataServices" :key="data.id">
                                <td>
                                    <span >{{data.code_chat}}</span>
                                </td>
                                <td>
                                    <span class="bold text-success">{{data.subject}}</span>
                                </td>
                                <td>
                                    <span class="badge" :class="data.class_service">{{data.service}}</span>
                                </td>
                                <td>
                                    <span>{{timeAgo(data.created_at)}}</span>
                                </td>
                                <td>
                                    <el-select v-model="data.status" @change="updateData(data.status,data.id,data.code_chat)" placeholder="Thay đổi trạng thái hỗ trợ">
                                        <el-option
                                ...
                                <td>
                                    <router-link :to="{ path: '/support/chat/'+ data.code_chat}" class="btn btn-primary btn-sm">Mở hộp thoại</router-link>
                                </td>
                            </tr>
                            </tbody>
                            <!--end::Tbody-->
...
export default {
    ...
    methods: {
        updateData(status,id,code) {
            let data = {
                status : status,
                id : id,
                code_chat : code
            }
            axios.post('/admin/support/update', data)
            toastr.success("Bạn vừa chuyển đổi thành công trạng thái của mã hỗ trợ " +code)
        },
        fetchData() {
            let obj = this
            let data = {
            ...
            axios.post('/admin/support/data?page=' + obj.pagination.current_page, data).then(res => {
